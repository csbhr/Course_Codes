...
def read_AAAI_file_text_term_list(root_path):
    text_data = ftools.read_csv_list(root_path + "AAAI-14 Accepted Papers - Papers.csv", header=[0])
    text_term_list = []
    for line in text_data:
        line_content = str(line[3]) + "-" + str(line[4]) + "-" + str(line[5])
...
def read_AAAI_dictionary(root_path):
    text_term_list = read_AAAI_file_text_term_list(root_path)
    dictionary = []
...
def read_AAAI_feature_vector(root_path):
    text_term_list = read_AAAI_file_text_term_list(root_path)
...
def read_AAAI_weight(root_path):
    text_term_list = read_AAAI_file_text_term_list(root_path)
    dictionary = ftools.read_txt(root_path + "dictionary.txt")
...
def action_AAAI_pca(root_path, origin_filename):
    print("---正在读取输入矩阵------------------")
